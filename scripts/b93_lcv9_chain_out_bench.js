...
async function main() {
  ...
  sec('11 every SAY line on Part One\'s walk card, on the real WhatsApp lane, in one thread (one database)');
  {
    ...
    T('11.9 SAY "The booking is confirmed" (no client): LEFTOVER until B35 is his; HEARD AS TURN 6 (no act at all), and BESIDE it the tidier request this cell used to drive', one(await say('The booking is confirmed', HEARD('{"acts":[],"route":"none"}'))).split('\n')[0] === DL.LINES.LEFTOVER && one(await say('The booking is confirmed', req([{ act: 'booking_confirmed' }]))).split('\n')[0] === DL.LINES.LEFTOVER);
