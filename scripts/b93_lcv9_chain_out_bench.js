...
function exits(DL, LH, GLITCH) {
  ...
  return [
    ...
    { why: 'uncovered', message: 'The booking is confirmed', ear: earOf(req([{ act: 'booking_confirmed' }])), is: isLeft, byte: 'LEFTOVER', label: 'a covered act naming NO client (B35 is with the founder)' },
    ...
    { why: 'attach_unsayable', message: 'Attach a package to Walk P8 Dated', ear: earOf(req([{ act: 'attach_package', client_as_spoken: 'Walk P8 Dated' }])), is: (r) => r === DL.LINES.B30, byte: 'B30', label: 'no package named (B31 enters with Part Two)' },
    { why: 'attach_no_lead', message: 'Attach Photographs and film to Walk P8 Datd', ear: earOf(req([{ act: 'attach_package', client_as_spoken: 'Walk P8 Datd', package_as_spoken: 'Photographs and film' }])), is: (r) => r === 'Could not attach the package. No lead called Walk P8 Datd. Add the lead first.', byte: 'B32' },
...
async function main() {
  ...
  T('1.2 B31, B33 and B35 are FREE: Part Two brings B31 and B33, and B35 is with the founder', !('B31' in DL.LINES) && !('B33' in DL.LINES) && !('B35' in DL.LINES));
  ...
  const whys = new Set();
  for (const x of EX) {
    const db = makeDb(world(), x.dbOpts);
  ...
  T('3.2 the exits driven above are EVERY reason preTurn can return but `empty` (3.3), read from the source', [...srcWhys].filter((w) => w !== 'empty').every((w) => whys.has(w)) && srcWhys.size === 13 && (src('src/lib/vendor/workingDoor.js').match(/return CHAIN\(/g) || []).length === 14);
  ...
  sec('11 every SAY line on Part One\'s walk card, on the real WhatsApp lane, in one thread (one database)');
  {
    ...
    T('11.1 SAY "Hello": "I didn\'t catch a task in that. You can say things like:" and two of the four examples', parts.length === 3 && parts[0] === DL.LINES.LEFTOVER && parts.slice(1).every((e) => coveredEx.includes(e)));
    T('11.2 SAY "Block 20 March, personal": B34', one(await say('Block 20 March, personal', req([{ act: 'block_date', date_as_spoken: '20 March' }]))) === 'I cannot do that by message yet. Use the app for it.');
    T('11.3 SAY "What\'s due this week?": B34', one(await say("What's due this week?", req([{ act: 'whatsdue' }], 'search'))) === 'I cannot do that by message yet. Use the app for it.');
    const leadsBefore = db.tables['public.leads'].length;
    T('11.4 SAY "Add a new lead Walk P8 Phone, 9876543210": B34, and the lead is NOT filed', one(await say('Add a new lead Walk P8 Phone, 9876543210', req([{ act: 'lead', client_as_spoken: 'Walk P8 Phone' }]))) === 'I cannot do that by message yet. Use the app for it.' && db.tables['public.leads'].length === leadsBefore);
    ...
    T('11.6 SAY "Add a new lead Walk P8 Fresh, wedding on 20 February 2027": the door takes it, as before this cut', one(await say('Add a new lead Walk P8 Fresh, wedding on 20 February 2027', req([{ act: 'lead', client_as_spoken: 'Walk P8 Fresh', date_as_spoken: '20 February 2027' }]))) === 'Lead added: Walk P8 Fresh · 20 February 2027.');
    T('11.7 SAY "Attach Photographs and film to Walk P8 Frsh" (misspelt): B32 with HER spelling', one(await say('Attach Photographs and film to Walk P8 Frsh', req([{ act: 'attach_package', client_as_spoken: 'Walk P8 Frsh', package_as_spoken: 'Photographs and film' }]))) === 'Could not attach the package. No lead called Walk P8 Frsh. Add the lead first.');
    ...
    T('11.9 SAY "The booking is confirmed" (no client): LEFTOVER until B35 is his', one(await say('The booking is confirmed', req([{ act: 'booking_confirmed' }]))).split('\n')[0] === DL.LINES.LEFTOVER);
