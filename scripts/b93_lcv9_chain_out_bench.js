...
// LABELED AMENDMENT · CE-45 ELZ-1 cut 2b: B31 (his V13) re-pinned; the quote_send and relay_pwa exit rows removed (both now the door's own).
...
function exits(DL, LH, GLITCH) {
  ...
  return [
    ...
    // ROW REMOVED (CE-45 ELZ-1 cut 2b, labelled): quote_send is the DOOR'S since cut 2b (b118b holds it); `note` above is the uncovered act.
    ...
    // ROW REMOVED (CE-45 ELZ-1 cut 2b, labelled): the relay_pwa exit is DELETED (P6b's second half); the pwa lane frames a relay as WhatsApp does (b101 6.12).
    ...
    // ROW RE-AIMED OUT OF THE EXITS (CE-45 ELZ-1 cut 2c, F4, labelled): three packages of one name are now the DOOR'S B24, numbered (his V12);
    // no longer an exit, so the specimen count falls by one; its subject lives on in cell 3.1b below.
...
async function main() {
  ...
  T('1.2 B31, B33 and B35 are HIS, present and hash-carried', DL.LINES.B31 === 'Which package for {client}? {list}. Reply with the number.' /* cut 2b: V13, his */ && DL.LINES.B33 === 'Could not attach the package. {package} has no fee yet. Set its fee in the app first.' /* cut 2a: V5, his */ && DL.LINES.B35 === 'Which client? Say the name.' && DL.LINE_HASHES.B35 === '7b73fec4bc3c30e66b5e33232961ccb26549d42d440d466e6e8de54402c1c773' && DL.LINE_HASHES.B31 === '1e5cb75ba7131c768dbd07546b5905b6cfe09021a97efc586c145341978406b2' && DL.LINE_HASHES.B33 === '30b99efbf542ea00089afdfba252db15b8f488325188ff8fe9c617524d22aa44' /* cut 2a: V5 */);
  ...
  { // 3.1b · the three-same-name specimen RE-AIMED (CE-45 ELZ-1 cut 2c, F4, labelled): its subject lives as the door's own B24, numbered, not an exit
    const db3 = makeDb(world());
    const out3 = await decide(db3, { message: 'Attach Tri to Walk P8 Dated', ear: earOf(req([{ act: 'attach_package', client_as_spoken: 'Walk P8 Dated', package_as_spoken: 'Tri' }])) });
    T('3.1b three packages of one name (was attach_unsayable): the DOOR answers his B24 numbered for three (F4), no exit', out3.door === true && Array.isArray(out3.keys) && out3.keys[0] === 'B24' && /^3 packages are called Tri: 1\. Tri \(Rs [\d,]+\) 2\. Tri \(Rs [\d,]+\) 3\. Tri \(Rs [\d,]+\)\. Reply with the number\.$/.test(out3.reply));
  }
  const srcWhys = new Set((src('src/lib/vendor/workingDoor.js').match(/CHAIN\([^,]+, '([a-z_]+)'/g) || []).map((m) => /'([a-z_]+)'/.exec(m)[1]));
  T('3.2 the exits driven above are EVERY reason preTurn can return but `empty` (3.3), read from the source', [...srcWhys].filter((w) => w !== 'empty' && w !== 'question').every((w) => whys.has(w)) && srcWhys.size === 22 && /* RE-PINNED (CE-45 ASK-1 cut 1, labelled): 'question' joins (22 reasons), reached only with a lane's ask switch ON and driven in b130a; no `return CHAIN(` added */ (src('src/lib/vendor/workingDoor.js').match(/return CHAIN\(/g) || []).length === 32); /* RE-PINNED (CE-45 ASK-1 cut 1, labelled): the uncovered exit reads `return (questionTurn(heard) && await uncoveredAsk(...)) || CHAIN(st.ear, 'uncovered')`, the same reason, one `return CHAIN(` fewer by shape */ /* RE-PINNED (CE-45 ELZ-1 cut 2b, labelled): relay_pwa's CHAIN deleted, one reason and one return fewer */ // RE-PINNED (CE-45 LCV-14, P7 cut 4 fix, labelled): the new-leads branch gains the head-count's refusal and the tally branch its render refusal (34 returns; no new reason); // RE-PINNED (CE-45 LCV-14, P7 cut 4, labelled): lookup_unsayable joins (22 reasons); the gate's own `return CHAIN(` became `return (await lookupDoor(...)) || CHAIN(` (one fewer), lookupDoor adds four (32 returns); // RE-PINNED (CE-45 LCV-14, P7 cut 3, labelled): team_unsayable and assign_many join; four returns added (team_unsayable, assign_many and book_no_lead in the team probe; the team rebuild's one): 21 reasons, 29 returns; // RE-PINNED (CE-45 LCV-13, P7 2b, labelled): cal_mixed joins; five returns added (cal_mixed, book_no_lead and calendar_unsayable from the move/cancel hook, calendar_unsayable twice from the B53 answer): 19 reasons / 25 returns // RE-PINNED (CE-45 LCV-12, P7 2a): lookup, book_no_lead, calendar_unsayable join (18 reasons / 20 returns) // RE-PINNED (CE-45 LCV-11, P6b): two lead_phone returns left, ONE came back for the two-runs case, relay_unsayable and relay_pwa came: 16 returns, 15 reasons; // 15 since LCV-10 B-2 (first cut): the phone guard is also applied at the name question, same reason lead_phone
  ...
  T('8.2 the switch is read in exactly ONE place: workingDoor.js standIn; neither lane names the key outside a comment or reads laneFlags for it (RE-AIMED, LSP_1: read NOWHERE)', keyReaders.length === 0 && (wdS.match(/readLaneFlag/g) || []).length === 1 && /readLaneFlag\(supabase, flag\)/.test(wdS)); /* RE-AIMED (CE-45 ASK-1 cut 1, labelled; F-E): the retired key is read nowhere; the door's ONE lane-flag read is the question agent's (askContext, ASK_FLAGS) */
  ...
  await mut('10.15 M13 planInvoice telling a FAILED read from no binder no longer: a failed read would speak B15: reddens 3.1', 'src/lib/vendor/workingDoor.js', [['  if (error || !Array.isArray(data)) return null;\n  // CE-45 ELZ-1 cut 2c: a picked binder', '  if (error || !Array.isArray(data)) return { noBinder: true, name };\n  // CE-45 ELZ-1 cut 2c: a picked binder']] /* cut 2c, labelled (c-45.54): the anchor follows the line 2c rewrote; the mutation unchanged */, [],
