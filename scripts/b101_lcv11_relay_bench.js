...
async function main() {
  ...
  T('4.16b [boundary, FLIPPED at ELZ-1 cut 2b] THE CARD (step 7, LCT row 6 replayed): quote_send is covered; a lead with no package reads B39 (his V6), nothing composed, nothing stored', r.reply === 'Could not send the quote. Asha Walk Fifteen has no package yet. Attach a package to Asha Walk Fifteen first.' && draftsIn(db).length === 0); /* LABELED AMENDMENT · CE-45 ELZ-1 cut 2b (P6b's second half, R-45.17): this boundary cell flipped as designed */
  ...
  await mut('6.6 MUTATION: relay struck from COVERED, the record\'s sentence reads B34 (4.1 red)', WDf, /* P7 2a re-aim: COVERED's literal grew a tail */ /* P7 cut 3 re-aim (CE-45 LCV-14, labelled): and again, ...TEAM_ACTS */ [["'lead', 'attach_package', 'relay', 'quote_send', ...CALENDAR_ACTS, ...TEAM_ACTS]", "'lead', 'attach_package', 'quote_send', ...CALENDAR_ACTS, ...TEAM_ACTS]"]] /* ELZ-1 cut 2b re-aim (labelled): COVERED gained quote_send */, [], async (rq) => { const d = makeDb(world()); const o = await turn(d, WR11_SAID, rec(WR11_JSON), { M: rq(WDf) }); return o.reply; }, (v) => v === B34);
  ...
  T('6.12 [FLIPPED at ELZ-1 cut 2b] THE CARD (step 12): the pwa lane stages and frames the relay as the WhatsApp lane does (B37), one draft stored, nothing sent', r.keys === 'B37' && draftsIn(db).length === 1 && r.out.why !== 'relay_pwa'); /* LABELED AMENDMENT · CE-45 ELZ-1 cut 2b (P6b's second half, R-45.17): this boundary cell flipped as designed */
  ...
  T('6.13 [FLIPPED at ELZ-1 cut 2b] a bare yes on the pwa lane with a draft open and no note: the frame is RE-SHOWN once (as on WhatsApp), the row untouched, nothing sent', r.keys === 'B37' && d0(db).state === 'staged' && sent.length === n0); /* LABELED AMENDMENT · CE-45 ELZ-1 cut 2b (P6b's second half, R-45.17): this boundary cell flipped as designed */
