...
async function main() {
  ...
  await mut('6.6 MUTATION: relay struck from COVERED, the record\'s sentence reads B34 (4.1 red)', WDf, /* P7 2a re-aim: COVERED's literal grew a tail */ /* P7 cut 3 re-aim (CE-45 LCV-14, labelled): and again, ...TEAM_ACTS */ [["'lead', 'attach_package', 'relay', ...CALENDAR_ACTS, ...TEAM_ACTS]", "'lead', 'attach_package', ...CALENDAR_ACTS, ...TEAM_ACTS]"]], [], async (rq) => { const d = makeDb(world()); const o = await turn(d, WR11_SAID, rec(WR11_JSON), { M: rq(WDf) }); return o.reply; }, (v) => v === B34);
