...
ok('§2.1 the demo select reads whatsapp_phone — the mint can ask who the phone belongs to',
  /\.select\('id, display_name, category, city, ig_handle, rate_display, photos, about, whatsapp_phone'\)/.test(FEED_CODE));
...
ok('§2.3 the filter runs at the MINT — before shapedDemo, so no mount can re-admit the row',
  /const shapedDemo = \(demoVendors \|\| \[\]\)\.filter\(v => !suppressedDemoIds\.has\(v\.id\)\)\.map\(/.test(FEED_CODE));
...
ok('§2.4 whatsapp_phone never appears inside the EMITTED demo shape — read at the mint, never on the wire',
  !/whatsapp_phone/.test(FEED_CODE.slice(FEED_CODE.indexOf('const shapedDemo'))));
...
const MUTATIONS = [
  ['the mint filter removed (mounts left to fend for themselves)',
   'const shapedDemo = (demoVendors || []).filter(v => !suppressedDemoIds.has(v.id)).map(',
   'const shapedDemo = (demoVendors || []).map(', '§2.3'],
...
];
{
  ...
  for (const [label, from, to, cell] of MUTATIONS) {
    ...
    const reddens =
      (cell === '§2.3' && !/const shapedDemo = \(demoVendors \|\| \[\]\)\.filter/.test(mutated)) ||
      (cell === '§2.2' && !/phoneForms\.push\(p, `\+\$\{p\}`\)/.test(mutated)) ||
      (cell === '§2.1' && !/, about, whatsapp_phone'\)/.test(mutated));
