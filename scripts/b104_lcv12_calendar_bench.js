...
async function main() {
  ...
  T('2.1 COVERED is ten; NEEDS_CLIENT names the acts that must name a client, and block_date and unblock_date are not among them', WD.COVERED.length === 14 /* RE-PINNED (CE-45 LCV-14, P7 cut 3, labelled): assign_crew and payment_reminder joined; b106 holds them */ /* RE-PINNED (CE-45 LCV-13, P7 2b, labelled): edit_event and cancel_event joined; b105 holds them */ && ['block_date', 'unblock_date', 'book_event'].every((a) => WD.COVERED.includes(a)) && !WD.NEEDS_CLIENT.includes('block_date') && !WD.NEEDS_CLIENT.includes('unblock_date') && WD.NEEDS_CLIENT.includes('book_event') && WD.NEEDS_CLIENT.includes('lead'));
