...
async function main() {
  ...
  T('2.1 COVERED is ten; NEEDS_CLIENT names the acts that must name a client, and block_date and unblock_date are not among them', WD.COVERED.length === 15 /* LABELED AMENDMENT · CE-45 ELZ-1 cut 2b: quote_send joined COVERED (fifteen kinds) */ /* RE-PINNED (CE-45 LCV-14, P7 cut 3, labelled): assign_crew and payment_reminder joined; b106 holds them */ /* RE-PINNED (CE-45 LCV-13, P7 2b, labelled): edit_event and cancel_event joined; b105 holds them */ && ['block_date', 'unblock_date', 'book_event'].every((a) => WD.COVERED.includes(a)) && !WD.NEEDS_CLIENT.includes('block_date') && !WD.NEEDS_CLIENT.includes('unblock_date') && WD.NEEDS_CLIENT.includes('book_event') && WD.NEEDS_CLIENT.includes('lead'));
  ...
  // 3.1 and 3.2 RE-AIMED (CE-45 LCV-14, P7 cut 4, labelled): the lookup is ANSWERED now (the new leads, B69 or B70; b108 holds the lines); what this cell
  // pins is unchanged in strength: a lookup on route search is never a job, never B18, keeps no note, writes nothing.
  T('3.1 row 13 C1 asis AS RECORDED ("lead", no client, route search): the new leads (B69 or B70, P7 cut 4), NEVER B18, no note, nothing written', ['B69', 'B70'].includes(r.keys) && r.out.why === 'lookup_leads' && noteOf(db) === undefined && db.log.inserts.filter((i) => i.table === 'public.leads').length === 0);
  db = makeDb(calWorld()); r = await turnC(db, S[13], heard('13/C2/asis'));
  T('3.2 row 13 C2 asis AS RECORDED (find, route search): the new leads (B69 or B70, P7 cut 4), never a job', ['B69', 'B70'].includes(r.keys) && r.out.why === 'lookup_leads');
  ...
  T('6.8 a name that is no lead of hers: B76 "No lead called Verma. Add Verma as a lead first, here or in the app." (his), exit book_no_lead, nothing written', r.reply === 'No lead called Verma. Add Verma as a lead first, here or in the app.' && r.keys === 'B76' && r.out.why === 'book_no_lead' && shoots(db).length === 0);
  ...
  // RE-AIMED (CE-45 LCV-15, LSP_2, labelled): R-45.16 supersedes F-44.109's drop. The preview asks again, in HIS words (B84, read from doorLines, never a literal
  // in vendorInbound.js), and the door answers it. What stays pinned: Victor's dropped sentence never returns.
  T('8.1 vendorInbound.js holds no "save all" sentence and the preview ends after its list (RE-AIMED, LSP_2: no Victor sentence; the preview ends with B84 from doorLines)', !/Reply "save all" to add them all, or tell me which to skip/.test(src(VIf)) && !/Reply "save all"/.test(src(VIf)) && /lines\.join\('\\n'\) \+ '\\n\\n' \+ require\('\.\/vendor\/doorLines'\)\.LINES\.B84/.test(src(VIf)));
  ...
  // LABELED AMENDMENT · CE-45 ELZ-1 cut 2b (F-44.175, c-45.54): the anchor carries lookupDoor's new `message` argument; the mutation is unchanged.
  await mut('9.4 MUTATION: the route-search gate removed reddens 3.1: row 13 C1 asks B18 for a lookup', WDf, /* anchor RE-AIMED (CE-45 LCV-14, P7 cut 4, labelled): the gate line now carries the lookups' door; removing it whole is the same mutation */ [["    if (heard && heard.route === 'search') return (await lookupDoor(supabase, vendor, heard, nowMs, L, st, message)) || CHAIN(st.ear, 'lookup'); // P7 cut 4: the lookups' door\n", '']], [], async (rq) => { const d = makeDb(calWorld()); const x = await turnC(d, S[13], heard('13/C1/asis'), { M: rq(WDf) }); return x.keys; }, (k) => k === 'B18');
  ...
  const ELEVEN = { B40: '1bec09f1e5d35c0c197a9229bba817744e1f995133bce2fcae722c5093e5eee3', B41: '6d882045d5fca760cce1f3dd1393bc6b6f3b23cea11f59a810dc910a51ea0b03', B42: 'aaaa2e6e807e81c3daa4619669eb8bcfb0cefeb65ca82a86123252a9d8642c3d', B43: 'b78f22148ed8f80aa71e2ba0e5311fb8f7d1d8798fd5b8631befb263924db0ad', B44: '2e8457a7173407d2b6166c895ad03188a19a174e3307a008e084996354ede0e5', B45: '4a359fa05de98e63fa577fac29fd25e48d37e72ebb3bec3d95561f47a584554b', B46: 'e06955310af567757977dc641dd8231d410bc582014e0ea62d4cb67ffbd820f1', B54: 'fc952e30b355667df5891a96d09a99894f5f1100c026af515a41a5dbf69cc6ab', B75: '279f87cbc350c71bc23be93f6c359051c57e7dc76d1bb8607b85122a5a66f261', B76: '90befbeea38250f99dc0743161e1b191aa45aea7bf3938cb9e7ccd8ee8178bb8', B77: '756908b48584d5cfbedc716eb21307d3c7a50fd7d3ad3210677e6f3bfe8a3a6c' };
  T('10.2 LINES holds 52: the eleven of his, hash-carried, each byte its literal; B41 to B45 byte-identical to blockHands.js\'s six-line register words; B75 to calendarSignals.js :134', Object.keys(DL.LINES).length === 83 /* RE-PINNED (CE-45 ELZ-1 cut 2b, labelled): B86 and B87 his (F-44.175's stopgap), LINES 83 */ /* RE-PINNED (CE-45 LCV-15, LSP_2, labelled): 79 to 81, B84 and B85, his (R-45.16, the screenshot save); b112 holds them */ /* RE-PINNED (CE-45 LCV-14, P7 cut 4 fix, labelled): 79 since B80 to B83, his; b109 holds them */ /* RE-PINNED (CE-45 LCV-14, P7 cut 4, labelled): 75 since B69 to B74, B78, B79, his (ASK 7: each form its own key); b108 holds them */ /* RE-PINNED (CE-45 LCV-14, P7 cut 3, labelled): 67 since B56 to B62, B67, B68, his (B60 carried, unspoken); b106 holds them */ /* RE-PINNED (CE-45 LCV-13, P7 2b, labelled): B48 to B53 joined, his; b105 holds them */ && Object.keys(ELEVEN).every((k) => DL.LINE_HASHES[k] === ELEVEN[k] && sha(DL.LINES[k]) === ELEVEN[k]) && src('src/lib/vendor/blockHands.js').includes("was already blocked. Nothing changed.") && /* RE-AIMED (CE-45 LCV-15 LSP_1, labelled): calendarSignals.js is DELETED (A11); B75's byte is pinned against its FIXED historical blob (C-44.7) */ require('child_process').execSync('git show 89e3a6e:src/lib/vendor/calendarSignals.js', { cwd: P('.') }).toString().includes("Couldn't put that on the calendar — nothing was changed.") /* RE-AIMED (CE-45 ELZ-1 e-151, the chair's ruling (a), labelled): the FIXED historical blob keeps its dashed byte; B75 is that byte with the em dash set as a full stop (R-45.30), and its live home (chat.js) carries the same new byte: one home, one byte */ && DL.LINES.B75 === "Couldn't put that on the calendar. Nothing was changed." && src('src/api/vendor-engine/chat.js').includes("`Couldn't put that on the calendar. Nothing was changed.`") && src('src/lib/vendor/blockHands.js').includes("`Couldn't block ${day}. Nothing was written.") && src('src/lib/vendor/blockHands.js').includes("`Couldn't unblock ${day}. Nothing was written."));
  ...
  { const d = makeDb(calWorld()); let x;
    ...
    T('11.13 THE CARD, STEP 13: "Book the Walk Seventeen Zeta shoot on 9 January 2027" → "No lead called Walk Seventeen Zeta. Add Walk Seventeen Zeta as a lead first, here or in the app."', x.reply === 'No lead called Walk Seventeen Zeta. Add Walk Seventeen Zeta as a lead first, here or in the app.' && shoots(d).filter((e) => e.event_date === '2027-01-09').length === 0);
