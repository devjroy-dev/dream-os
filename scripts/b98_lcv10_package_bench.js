...
async function main() {
  ...
  await mut('8.2 M2 the door GUESSING the one package she holds instead of asking B31 (reddens 2.2)', WDf,
    [["    if (!said) { const line = DL.whichPackage(pkgs.map((p) => p && p.name), client); return line ? { speak: line, key: 'B31', skipHarvest: true } : null; }\n", ""], ["    const hits = pinned.length === 1 ? pinned : pkgs.filter((p) => p && key(p.name) === key(said));", "    const hits = pinned.length === 1 ? pinned : (!said && pkgs.length === 1 ? pkgs : pkgs.filter((p) => p && key(p.name) === key(said)));"]] /* cut 2c, labelled (c-45.54): 2c put the pinned-package lines between the two anchored lines; the same mutation in two anchored parts */, [],
