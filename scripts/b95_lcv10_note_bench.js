...
async function main() {
  ...
  const HOSTILE = [undefined, null, 0, -1, NaN, '', 'x', true, [], {}, () => {}, Symbol('s'), 10n, trap, getter('asked'), getter('acts'), getter('tries'),
    ...
    { asked: 'B26', acts: [{ act: 'note', client_as_spoken: 'A' }] } /* P7 2a re-aim: the uncovered act is assign_crew; block_date is covered */ /* RE-AIMED (CE-45 LCV-14, P7 cut 3, labelled): assign_crew is covered; the uncovered act is `note` */, { asked: 'B26', acts: [{ act: 'attach_package' }] }, { asked: 'B26', acts: [att('A', 'B'), att('A', 'B'), att('A', 'B'), att('A', 'B'), att('A', 'B')] },
  ...
  await mut('9.5 M5 the note read BELOW the bare yes-or-no exit: her "Yes" to B26 meets yes_no_nothing_waiting and LEFTOVER (reddens 3.10)', WDf,
    // LABELED AMENDMENT · CE-45 ELZ-1 cut 2b (c-45.54: a bench anchors itself): 2b's relay line reads `if (!note && !live && said !== null) {`
    // too, so the bare anchor matched TWO sites and hit the wrong one. M5 is anchored by the bare yes-or-no exit's own context (the open-draft
    // block's catch and its close, the lines that precede it); 9.5a asserts that anchor matches exactly one site.
    [["      } catch (e) { try { console.warn('[door:open draft]', e && e.message); } catch (_e) { /* */ } }\n    }\n    if (!note && !live && said !== null) {", "      } catch (e) { try { console.warn('[door:open draft]', e && e.message); } catch (_e) { /* */ } }\n    }\n    if (!live && said !== null) {"]], [],
    async (rq) => flow(rq, [ASK, ['Yes', JSON.parse(NONE_JSON)]]), (x) => x.out[1].keys === 'LEFTOVER');
  T('9.5a M5\'s context anchor matches EXACTLY ONE site in workingDoor.js (c-45.54)', src(WDf).split("      } catch (e) { try { console.warn('[door:open draft]', e && e.message); } catch (_e) { /* */ } }\n    }\n    if (!note && !live && said !== null) {".replace(/\\n/g, '\n')).length === 2);
  ...
  await mut('9.13 CONTROL: noteAct\'s covered test ALONE removed: a planted note over note (P7 cut 3 re-aim, CE-45 LCV-14, labelled: assign_crew is covered) is still refused, by allCovered', WDf,
    [["typeof a.act !== 'string' || !COVERED.includes(a.act)) return null;", "typeof a.act !== 'string') return null;"]], [],
    async (rq) => rq(WDf).validNote({ asked: 'B26', acts: [{ act: 'note', client_as_spoken: 'A' }] }), (v) => v === null);
