...
async function main() {
  ...
  sec('7 · MUTATIONS OF PRODUCTION CODE, each must redden');
  {
    ...
    await M('7.1 M1 the lookups\' door removed (the gate back to B34 alone) → row 13 C2 reads B34, not B69', WDf, [['return (await lookupDoor(supabase, vendor, heard, nowMs, L, st, message)) || CHAIN(st.ear, \'lookup\');', 'return CHAIN(st.ear, \'lookup\');']],
