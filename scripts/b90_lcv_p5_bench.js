...
async function main() {
  ...
  const RULED = {
    ...
    // P7 cut 4 (CE-45 LCV-14, labelled): the eight lookup bytes join the ruled set (ASK 7: each form its own key); one cell each in the RULED loop; b108 holds them
  ...
  const sseDoor = cj.slice(cj.indexOf('      const doorOut = await doorTurn(req, llmWiring, message, roomAssert);'), cj.indexOf('      // CE-45 · LCV-16 · LSP_5 · L5-d: the six estate fact reads that stood here')); // RE-ANCHORED (CE-45 LCV-16 LSP_5, labelled): L5-d deleted the old end anchor, the slice ran to the file's end and 8.1 to 8.3 read the tail (8.2 and 8.3 were hollow greens); the note L5-d left in its place ends the door turn now. // RE-ANCHORED (LSP_1, labelled): the old end, `req._lcvEar`, is deleted; the SSE door block now ends where the Advisor turn's reads begin
  T('8.0 both door slices are bounded by live anchors (a missing end anchor reddens here, never a hollow green below)', sseDoor.length > 0 && sseDoor.length < 6000 && cj.includes('      // CE-45 · LCV-16 · LSP_5 · L5-d: the six estate fact reads that stood here') && cj.includes('\n    // CE-45 · LCV-16 · LSP_5 · L5-d: the six estate fact reads left with the business room'));
  ...
  const jsonDoor = cj.slice(cj.indexOf('\n    const doorOut = await doorTurn(req, llmWiring, message, roomAssert);'), cj.indexOf('\n    // CE-45 · LCV-16 · LSP_5 · L5-d: the six estate fact reads left with the business room')); // RE-ANCHORED (CE-45 LCV-16 LSP_5, labelled): as 8.1 // RE-ANCHORED (LSP_1, labelled): as 8.1
