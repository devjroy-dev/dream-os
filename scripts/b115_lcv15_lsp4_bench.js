...
(async () => {
  ...
  T('1.4 engine.js is the couple lane alone: 737 lines or fewer, one exported name', en.split('\n').length <= 740 && JSON.stringify(Object.keys(require(P(EN)))) === '["runCoupleAgenticTurn"]');

  sec('2 the couple lane, byte-identical (C-44.7, hashes from cd71986)');
  for (const f of ['runCoupleAgenticTurn', 'relayAttributionPrefix', 'markRelayProvenance', 'mergeSameRole']) T(`2.1 ${f} is byte-identical to cd71986`, sha(body(en, f)) === PIN[f]);
