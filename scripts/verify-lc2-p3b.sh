#!/usr/bin/env bash
# scripts/verify-lc2-p3b.sh — TDW CE-43 · LC-2 packet 3b (dream-os, F-43.86) · the founder's ONE verify command.
#
#   bash scripts/verify-lc2-p3b.sh
#
# WHY A FILE. Packet 3b changes no engine TypeScript, but the build still runs first so the
...
#   3  b83 (this packet, amended by label), b47, b51, b40 (the readers of the two changed homes)
#   4  bash scripts/run-floor.sh --delivery scripts/floor-manifest-lc2-p3b.txt --check
...
say "2/4 node --check"
for f in \
  src/lib/vendor/invoices.js src/lib/vendor/schedules.js src/api/vendor/invoiceSchedule.js \
  scripts/b83_lc2_p3_promotion_bench.js; do
  node --check "$f"
  rc=$?
...
say "3/4 benches"
for b in b83_lc2_p3_promotion_bench b47_money_crossing_bench b51_invoice_document_bench b40_victor_sitting_bench; do
...
bash scripts/run-floor.sh --delivery scripts/floor-manifest-lc2-p3b.txt --check
rc=$?
