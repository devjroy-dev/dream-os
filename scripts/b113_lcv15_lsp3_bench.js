...
(async () => {
  ...
  T('2.3 coupleDrafts.js is untouched (its blob at 90f607d); relayToCouple.js changed only by F-44.176 (the shared-line prefix)', (() => { /* RE-AIMED (CE-45 ASK-1 cut 1, labelled; d7): coupleDrafts gains ONE read function, sentFor, and its export; no byte before it moves. The cell now proves that: zero lines removed since 90f607d, and every line added is sentFor's (a SELECT, no write). */ const d = execSync('git diff -U0 90f607d -- src/lib/vendor/coupleDrafts.js', { cwd: ROOT, encoding: 'utf8' }).split('\n'); const minus = d.filter((l) => l.startsWith('-') && !l.startsWith('---')); const plus = d.filter((l) => l.startsWith('+') && !l.startsWith('+++')).join('\n'); return minus.length === 0 && /async function sentFor\(supabase, vendorId/.test(plus) && /^\+  sentFor,$/m.test(plus) && !/\.(insert|update|upsert|delete)\(/.test(plus); })() && /function sharedLinePrefix\(vendor, from, environment\)/.test(require('fs').readFileSync(require('path').join(ROOT, 'src/lib/vendor/relayToCouple.js'), 'utf8')));
