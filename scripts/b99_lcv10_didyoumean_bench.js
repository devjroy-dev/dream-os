...
async function main() {
  ...
  T('5.3 ONE vendor byte added, B36, his; LINES holds 38; the home is nearestName, called from exactly one offer helper', Object.keys(DL.LINES).length === 38 && (src(WDf).replace(/^\s*\/\/.*$/gm, '').match(/nearestName\(/g) || []).length === 2);
