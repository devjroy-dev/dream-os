#!/usr/bin/env bash
# scripts/verify-lc2-p3c.sh — TDW CE-43 · LC-2 packet 3c (dream-os, F-43.87) · the founder's ONE verify command.
#
#   bash scripts/verify-lc2-p3c.sh
#
# WHY A FILE. Packet 3c changes no engine TypeScript, but the build still runs first so the
...
#   3  b83 (amended by label), b82, b48, b43 (readers of the promotion doors)
#   4  bash scripts/run-floor.sh --delivery scripts/floor-manifest-lc2-p3c.txt --check
...
say "2/4 node --check"
for f in \
  src/lib/vendor/promotion.js scripts/b83_lc2_p3_promotion_bench.js; do
  node --check "$f"
  rc=$?
...
say "3/4 benches"
for b in b83_lc2_p3_promotion_bench b82_lc2_p2_packages_bench b48_engine_mounts_bench b43_solutions_doors_bench; do
...
bash scripts/run-floor.sh --delivery scripts/floor-manifest-lc2-p3c.txt --check
rc=$?
