...
async function main() {
  ...
  T('1.4 LINES 67; the nine hashes literal (B56 to B62, B67, B68); each byte hashes to its literal', DL.LINES && Object.keys(DL.LINES).length === 75 /* RE-PINNED (CE-45 LCV-14, P7 cut 4, labelled): 67 → 75, the eight lookup bytes; b108 holds them */ && Object.keys(NINE).every((k) => DL.LINE_HASHES[k] === NINE[k] && sha(DL.LINES[k] || '') === NINE[k]));
  ...
  T('1.7 the offer\'s slot is THREE-WAY in ONE table (package, member, else client), read by offerFor and the B36 re-ask', WD.slotField && WD.slotField('member') === 'member_as_spoken' && WD.slotField('package') === 'package_as_spoken' && WD.slotField('x') === 'client_as_spoken' && (src(WDf).match(/slotField\(/g) || []).length === 2);
