...
async function main() {
  ...
  T('1.1 COVERED holds assign_crew and payment_reminder (fifteen kinds since ELZ-1 cut 2b); NEEDS_CLIENT holds payment_reminder and not assign_crew', WD.COVERED.includes('assign_crew') && WD.COVERED.includes('payment_reminder') && WD.COVERED.length === 15 /* LABELED AMENDMENT · CE-45 ELZ-1 cut 2b: quote_send joined COVERED (fifteen kinds) */ && WD.NEEDS_CLIENT.includes('payment_reminder') && !WD.NEEDS_CLIENT.includes('assign_crew'));
  ...
  T('1.10 membersLine: two is his B61 verbatim by position; three is R-45.9\'s derived form', typeof DL.membersLine === 'function' && DL.membersLine('Harsh', [{ name: 'Harsh', role: 'editor' }, { name: 'Harsh', role: '4410' }]) === 'Two on your team are called Harsh: 1. Harsh (editor) 2. Harsh (4410). Reply with the number.' && DL.membersLine('Harsh', [{ name: 'Harsh', role: 'a' }, { name: 'Harsh', role: 'b' }, { name: 'Harsh', role: 'c' }]) === '3 on your team are called Harsh: 1. Harsh (a) 2. Harsh (b) 3. Harsh (c). Reply with the number.' && DL.membersLine('Harsh', [{ name: 'Harsh', role: 'a' }]) === null);
  ...
  {
    const d = db(estate()); d.tables['public.team_members'].push(tmRow({ id: '00000000-0000-4000-8000-0000000000c1', name: 'Harsh', role: 'editor' }), tmRow({ id: '00000000-0000-4000-8000-0000000000c2', name: 'Harsh', phone: '+919876544410' })); // cut 2c, labelled (F-44.178): same-named members are ordered by name then ID; fixed ids make that order this cell's, never a random UUID's
    const x = await turnX(d, 'Assign Harsh to the Walk Seventeen Alpha shoot', req([assign('Harsh', 'Walk Seventeen Alpha')]));
    T('3.15 two members of one name → B61 by position, {role} the row\'s role, else the phone\'s last four; nothing written', x.reply === 'Two on your team are called Harsh: 1. Harsh (editor) 2. Harsh (4410). Reply with the number.' && crewOf(d, SHOOT).length === 0);
    d.tables['public.team_members'].push(tmRow({ id: '00000000-0000-4000-8000-0000000000c3', name: 'Harsh', created_at: '2026-08-15T10:00:00Z' }));
    const y = await turnX(d, 'Assign Harsh to the Walk Seventeen Alpha shoot', req([assign('Harsh', 'Walk Seventeen Alpha')]));
    T('3.16 three → R-45.9\'s derived form; no role and no phone → the day added, full month', y.reply === '3 on your team are called Harsh: 1. Harsh (editor) 2. Harsh (4410) 3. Harsh (15 August 2026). Reply with the number.');
