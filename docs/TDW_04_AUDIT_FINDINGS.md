...
**THE VERDICT: the fork returned NEITHER — the write landed.** F-04.21 was not a phantom write. `public.leads d80aa837`, `source='victor'`, `budget_max=40000`, `wedding_date=2026-07-17`, `state='new'` — created BY the chat at 11:22:38, deleted by the founder from the list page at 12:30:34, correctly denied at 13:04. **CE ruling Q-B0-1 WITHDRAWN on this evidence.** Head (b) DISSOLVED (the denial was true). Head (a) restated as **false characterization of a real write over an unwitnessed confirmation** → Block 06, top shelf. **Item 2 died as a code rider** (nothing to fail-close against). Full record in FINDINGS_LOG's B0 section.

**The sentence the specimen exists to write:** *head (a) — the claim was true, but she didn't read; head (b) — the denial was true, but he didn't look. Right twice, by luck, in opposite directions.*

**SHIPPED:** F-04.25's cure (`cabinet.js:54` — the cabinet's events read filtered `deleted_at`, F-04.17's missing half; a soft-deleted future date is a **sellable** date) · item 3 (the chat lane joins `vendor_activity_log`, one row per nested mutating `donna_call`, `isErr`-gated — **CE extension, recorded as new scope, not laundered into ST-3d**) · item 4a (`donna_find` reaches `public.leads`, vendor-scoped, read-only, **fail-closed**; dispatch stays 06's) · item 4b (create + field-edit doors sync the snapshot; D-4's lying comment corrected) · the L-8 oracle written into the census doc as **runnable SQL** with three assertions (**witnessed green 125000 / 3 / 1**, matching the Part A seal) · `FINDINGS_LOG:562`'s NULL-unsafe prose struck (F-04.24) · `messages` ratified into the Vocabulary Audit (F-04.22) · `db/queries/engine_schema_dump.sql` (F-04.23's structural cure).

**GATES:** `node --check` PASS on all three `.js` · `tsc -p src/engine/tsconfig.json --noEmit` **EXIT=0**, gate proven live by probe (an injected type error was caught, then removed). Docs + SQL otherwise.

**OPEN:** one disclosed deviation standing for ratify-or-revert — item 4b resolves the agent in-handler because `resolveAgent()` is blocking (401/500) and the CE's own constraint forbids failing a lead creation. Two PROPOSALs (entity_id on tool_calls; door-side logging for the four signal-only tools). B0 seals on the cold proof turn.

## 14. SEAL — Part A sittings A4 + A4.1 (2026-07-15) — **PART A CLOSED**

**CE-22:** dream-os `1b87981` · dreamos-pwa `525b2c8` · greens witnessed. AddSheet draft-first · splash · L-10 executed (the last independent money arithmetic is dead — every renderer a consumer of `pendingOf`) · FilterRail+sort · F-04.14 ruled-returned and COMPLETED by F-04.20 (the bus's debt ledger — an invalidation with no listeners is remembered, not dropped) · F-04.19 closed · founder copy law swept to zero rendered persona strings, ruled confirm verbatim · the SW-LIVE pass run for the first time (4/4 on a real prod build under the fonts-stub ruling) · census green with the oracle line inherited permanently (1,25,000 / 1, three surfaces + SQL agreeing) and earning its keep on first run: F-04.21 (Swati Roy's 40k claimed-by-chat, held-by-nothing) → Block 06 with a named specimen · the Rahul Sharma trail read and closed: undo-tapped, system honest. Full detail incl. the executor's disclosure (a false bench-execution claim, owned) in FINDINGS_LOG. **Part A: A1 → A4.1, closed. Next: Part B.**

## 13. SEAL — Part A sittings A3 + A3.3 (2026-07-15)

**CE-22:** dream-os `923821e` · dreamos-pwa `afb1ba8` · both green, witnessed by the founder's smoke against the live deploys. **VERDICT-(d) IS DEAD, and then some:** A3 made hub ≡ Invoices (one derivation, two renderers — the obituary screenshot pair); the founder's phone then exposed the drawer's independent rule-set lying in BOTH directions (money undercount ₹85,000 / calendar over-claim 3-vs-1); A3.3 shipped the two CE-ratified predicates and the founder witnessed **three surfaces agreeing with his SQL to the rupee and the row** — `Rs 1,25,000` on drawer, slice, and hub; `ON THE CALENDAR: 1`; Dev Roy 2 and Keka visible on a money page for the first time. `derive.ts::pendingOf()` is now the sole money oracle; every renderer a consumer. The A3.2 interlude (pass skipped, blank list, ruled revert, causation OPEN as F-04.18 — SW hypothesis leading, unclosable on timing alone) and the ratified bench-gap promotion (SW-live pass scenario, block-wide) are in FINDINGS_LOG's A3 section in full. F-04.14 reverted-and-unruled; F-04.19 dormant old rule in today.js, logged. Part A remaining: **A4** (last sitting).

## 12. SEAL — Part A sitting A2 (2026-07-15)

**CE-22:** dream-os `b236db8` · dreamos-pwa `e5e2328` · Railway GREEN + Vercel GREEN, founder-confirmed. Executor browser pass 20/20 + touch pass 8/8 (regression-proofed) + sentinel cold-proof; founder phone smoke witnessed every charter item in production. **F-04.2's fixture consumed: `lead_delete 9566a91e` with `deleted_at` set — the DELETE button the ledger caught lying at 05:22 kills honestly at 08:14.** Undo is deferred-fire (documented mechanism); the one real reversal door (binder hide/unarchive) commits immediately and undoes through it. Remount verdict: REMOUNTS, recorded. Three hotpatches inside the sitting — toast centering (pre-existing), the notes mapper (executor defect), the panel-pager gesture conflict (executor harness blind spot; harness routes now live inside the shell). Open to CE: F-04.8 (events `state` door rider), F-04.9 (primer copy), F-04.12 (Mark-lost gating ruling), the P4-extras remainder. Full record in FINDINGS_LOG's A2 section.

## 11. SEAL — Part A sitting A1 (2026-07-15)

**CE-22:** dream-os `eeee188` · dreamos-pwa `a904424` · founder smoke ran against live prod. Executor browser pass 21 PASS + founder phone smoke 7/7 — full record in FINDINGS_LOG's A1 section. WishboneSheet live on both planes (one door, both callers; money refuses inline per the witnessed-door law; prefill-not-fire witnessed in production), L-1 lanes on every record surface, cabinet renames confirmed with real data, both riders (phoneKey degenerate guard ×2, no-op log guard) deployed. New findings F-04.5/6/7 filed with owners; F-04.2's fixture (`9566a91e`) still standing for A2.
