...
3. **A naming + placement call, made unilaterally, offered for revert.** This file is `docs/specs/TDW_04_B3_OPENING_PACKET.md`. Nothing ruled its name or its home. I chose `docs/specs/` because its direct predecessor in the chain (`TDW_04_B2_TO_B3_HANDOFF.md`) lives there and the CE will read them side by side; I chose "packet" because it is the estate's own word. **A better name or home is a one-line instruction and I will move it.**
4. **This packet is durable and pre-go.** It is committed to the repo before B3's charter is settled. **Every guard against F-04.40 I could apply, I applied**: the header states nothing shipped, the status line states nothing shipped, no proof is claimed, no seal is implied, and no artifact is described as existing that does not. **If a committed pre-go packet is itself the wrong instrument, say so and it becomes transport-only.**
