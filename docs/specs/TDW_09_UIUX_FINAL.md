...
| SCHEMA §vendors tier/trial fields + the tier rules | **CANON (founder-verbatim, 2026-08-07, supersedes this row's prior contents):** Free = no AI/chat · Essential Rs 999 pm · Signature Rs 1,999 pm · Prestige Rs 2,999 pm. **F-10.63 — THIS ROW WAS WRONG ON TWO OF THREE PRICES AND OMITTED FREE ENTIRELY:** it read *"Essential 499 / Signature 1,999 / Prestige 3,999"* — Essential low by Rs 500, Prestige high by Rs 1,000, in opposite directions, and no Free tier at all. Caught at the billing sitting's read-first, before any plan was created in the Razorpay dashboard; building from this row would have put three wrong prices in front of paying vendors. **F-10.64 — THE TRIAL LAW BELOW HAS EXPIRED AND IS UNIMPLEMENTED:** the prior text read *"before Aug 1 2026 = Signature free until Aug 1; after = 30-day trial → Essential"*. That boundary passed on 2026-08-01; the estate is now permanently on the "after" branch, and its acceptance bar at §"Sandbox Razorpay" (*"trial boundary unit tests pass both sides of Aug 1"*) no longer has two live sides. Neither branch was ever built, and per **F-10.27** it cannot be: `public.vendors` carries `tier` but NO trial start or end column and no `TRIAL_DAYS` constant exists in `src/`, so a 30-day trial has no clock to run on. The trial-vs-Free semantic (`trial` is time-bounded, canon-Free is permanent-no-AI — one cannot silently become the other) is **F-10.23's held founder ruling, gating the rename sitting**. The billing sitting (0114) deliberately writes canon tier names only, leaves legacy `trial` rows byte-untouched, and defaults `vendors.billing_status` to `'none'` rather than `'trial'` precisely so this question stays open for the founder rather than being answered by a DDL default. |
...
**Checkout — AMENDED AT THE BILLING SITTING (R-BILL.1 / R-BILL.2, 2026-08-07).** The vendor-lane checkout described in this paragraph is **NOT v1** and did not ship with 0114. v1's rail is **Razorpay Subscription Links**, created by the founder from the Razorpay dashboard and sent per vendor: real mandates, real `sub_…` ids filling `vendors.razorpay_subscription_id` honestly, **zero pwa bytes in either lane** (the vendor rehaul Phase B holds `app/vendor/**` + `lib/vendor/**` in dreamos-pwa), and the webhook still the sole writer. The founder plants the vendor's UUID in the link's **Notes** at creation; the webhook resolves on `notes.vendor_id`, then on the subscription id thereafter. The paragraph below stands as the **chartered v2** — a vendor-lane or native checkout, post-Phase-B or per TDW_11 — and it is a narrow surface swap over identical events, because nothing in the ledger or the flip changes when it lands. *Original v1 text, retained as the v2 design:* Settings tier masthead (tier in Cormorant, trial arc, price line) → `Upgrade` → backend `POST /billing/checkout` creates/reuses razorpay customer + subscription (plan IDs from env, founder's dashboard), Razorpay Checkout JS drop-in → success returns to a confirmation sheet. **The webhook is the truth:** `POST /webhook/razorpay` (signature-verified) → billing_events ledger → `subscription.activated/charged` flips `vendors.tier` + `billing_status`; `halted/cancelled` downgrades per the tier law (trial rules re-verified against today's date at build). UI never flips tier itself — it reflects the webhook.
