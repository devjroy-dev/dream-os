...
| C4 | Capacity defaults: photographer 1 · mua 2 · decorator 2 · florist 2 · delivery N/A · **planner: occupancy OFF until 04.5 (crew math lands there)** — vendor-editable always **[CORRECTION NOTE, B6-S2 2026-07-17, R-B6-27: THE CODE'S RULED TABLE STANDS — `occupancy.js` CATEGORY_CAPACITY = photography 1 · makeup 2 · decor 1 · venue 1 (Q-B3-2 as corrected, keyed on categoryProfiles' PROFILES space). This row's decorator 2 / florist 2 values predate the Q-B3-2 correction and the F-04.59 florist→decor merge; a `florist` key would resurrect a dead category. The surfaces paper's variant table (makeup 1 · florist 3 · other 1) is RETRACTED BY NAME as authored drift — those values appear in no ruled source, and an `other` key would silently turn occupancy ON for every unmapped category, breaking RULED_OFF's design. One home: the map in occupancy.js is the truth; this row is historical.]** |
