...
## 6. Gates
- **Before ANY cutover (MUST — F-05.6, CE-35):** OTP moves onto the Meta transport. PRIMARY (a, SEALED CE-36): OTP rides `metaCloud.sendMetaTemplate` when the lane is Meta-live (keys on `*_PHONE_NUMBER_ID` presence) — so provisioning `BRIDE_PHONE_NUMBER_ID` flips the lane's conversation AND its OTP onto Meta together. Founder step: file/approve the couple+circle auth template(s) + witness one live OTP (confirms the button shape — one-function flip ready). FALLBACK (b, sealed): `OTP_WA_NUMBER` (dedicated Twilio number) — set it as an emergency backstop; it survives M2b. Sites 1–3 (couple/circle) before the BRIDE cutover; sites 4–5 (vendor) before the VENDOR cutover. Post-cutover OTP rides the lane's Meta number as an auth template (opt-out-exempt, un-gated by F-05.2).
