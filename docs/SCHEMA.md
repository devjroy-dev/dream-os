...
**Latest migration applied:** 0077 (availability convergence — `vendor_availability` dropped into `public.events` kind='blocked'). Ladder tail at this correction: 0075 (events slots + the blocked UNIQUE partial index) · 0076 (vendors.slot_capacity) · 0077. Reservations run through 0095 (LD-8: holes harmless, renumbering forbidden).
