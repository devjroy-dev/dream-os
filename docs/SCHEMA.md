...
**Last updated:** 2026-07-17 (front page corrected by the CE at TDW_04 B6 kickoff — the doc-sweep item filed at the checker sitting and carried since. The body below this header is HISTORICAL NARRATIVE, last rewritten 2026-05-30; the schema itself has moved past it.)
**Session (historical):** Phases 1–3 vendor/bride build. New since 0062: 0063 (vendor_activity_log — cross-surface action log, Phase 1.5) and 0064 (vendors.base_fee_min/max — enquiry budget enrichment, Phase 2). Phase 3 (send_to_couple vendor reply path) added NO new tables — reuses conversations/messages/leads.
**Supabase project:** nvzkbagqxbysoeszxent (Mumbai, ap-south-1)
**Latest migration:** 0078 (per-slot blocking — the blocked UNIQUE key widened to (vendor_id, event_date, slot); full_day exclusivity enforced at eventWrite; the two 0075 blocked indexes retired inside the same file, R-B6-26). **0078 ships with the B6-S2 delivery and is FOUNDER-RUN — this line is true the moment its verify block passes in his editor; until then 0077 is the applied tail.** Ladder: 0075 (events slots) · 0076 (vendors.slot_capacity) · 0077 (availability convergence) · 0078. Reservations run through 0095 (LD-8: holes harmless, renumbering forbidden).
**⚠ THE WITNESSED TRUTH OUTRANKS THIS FILE.** For column-level facts, read `docs/db/PUBLIC_SCHEMA.md` (57 tables / 641 columns + constraints addendum, founder-run 2026-07-16) and `docs/db/ENGINE_SCHEMA.md` (25 tables / 242 columns). Founder-run SQL is written only against those witnessed lists — never against this file's prose (standing rule, F-04.23/F-04.57).
...
Couple-name drift (CE-19c, for TDW_03's merge tooling): exact-ilike lead matching cannot see that "Ritika and Arjun" is "Ritika" — the F4 ghost shape. The tool-description line (CE-19a) instructs enrichment under the name AS FILED; the general merge case is TDW_03's, arriving documented, not discovered.
