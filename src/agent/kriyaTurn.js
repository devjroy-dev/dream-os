...
async function runKriyaTurn(anthropic, supabase, vendorId, myraMessage, prior, onEvent, today) {
  ...
  const toolCalls = [];
  let spoken = null;
  let inTok = 0, outTok = 0, cacheRead = 0, cacheWrite = 0;  // Kriya's token usage this turn (carried up; cache buckets priced correctly)
  ...
  for (let i = 0; i < KRIYA_WORK_ITERS; i++) {
    ...
    if (spoken && listen && work.length) {
      ...
      return { reply: spoken, session: null, tool_calls: toolCalls, usage: { input_tokens: inTok, output_tokens: outTok, cache_read: cacheRead, cache_write: cacheWrite } };
    }
  }

  return { reply: spoken || '(no reply)', session: null, tool_calls: toolCalls, usage: { input_tokens: inTok, output_tokens: outTok, cache_read: cacheRead, cache_write: cacheWrite } };
}
